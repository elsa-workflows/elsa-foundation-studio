...
function Je(e) {
  e.settingEditors.add({
    id: "select",
    order: 10,
    supports: (t) => (t.options?.length ?? 0) > 0 || B(t, "select"),
    component: tn
  }), e.settingEditors.add({
    id: "boolean",
    order: 20,
    ...
    id: "json",
    order: 40,
    supports: (t) => ["object", "array"].includes(j(t.jsonType)) || B(t, "json") || B(t, "textarea"),
    component: an
  }), e.settingEditors.add({
    id: "secret",
    order: 50,
...
function qe() {
  const e = C(() => Pe(H), []), [t, a] = w("server"), [s, l] = w(() => ({
    studio: Z(),
    server: Z()
