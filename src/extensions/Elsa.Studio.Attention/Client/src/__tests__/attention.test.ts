...
describe("Studio Attention", () => {
  ...
  it("preserves rejected hosts as incomplete-source failures", async () => { const api = { host: { baseUrl: "studio", http: { getJson: async () => { throw new Error("offline"); } } }, backend: { baseUrl: "backend", http: { getJson: async () => ({ generatedAt: "2026-01-01T00:00:00Z", contributors: [] }) } }, runtime: { attention: { hostApiEnabled: true } } } as any; const snapshot = await loadAttention(api, new AbortController().signal); expect(snapshot.items).toHaveLength(0); expect(snapshot.failures[0]).toMatchObject({ contributorId: "host.studio", status: "unavailable" }); });
  it("treats an absent optional Studio API differently from an absent backend API", async () => {
    ...
    const available = { host: { baseUrl: "studio", http: { getJson: async () => missing() } }, backend: { baseUrl: "backend", http: { getJson: async () => backendResponse } }, runtime: { attention: { hostApiEnabled: true } } } as any;
