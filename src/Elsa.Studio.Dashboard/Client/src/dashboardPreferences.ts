  ...
  const known = contributions.map(widget => {
    ...
    if (widget.settings) {
      if (settings !== undefined && settingsSchemaVersion !== widget.settings.schemaVersion) {
        settings = widget.settings.migrate?.(settings, settingsSchemaVersion ?? 0) ?? widget.settings.defaults;
        settingsSchemaVersion = widget.settings.schemaVersion;
      }
      settings = widget.settings.validate(settings ?? widget.settings.defaults) ?? widget.settings.defaults;
      settingsSchemaVersion = widget.settings.schemaVersion;
    }
...
export class DashboardPreferenceStore {
  ...
  async load(): Promise<DashboardPreferenceDocument> {
    ...
    try {
      return await this.api.backend.http.getJson<DashboardPreferenceDocument>(`/_elsa/studio/preferences/${dashboardPreferenceNamespace}`, { headers: this.headers() });
    } catch (error) {
  ...
  async save(document: DashboardPreferenceDocument): Promise<DashboardPreferenceDocument> {
    ...
    const headers = { ...this.headers(), ...(document.revision ? { "If-Match": `"${document.revision}"` } : { "If-None-Match": "*" }) };
    try {
      return await this.api.backend.http.putJson<DashboardPreferenceDocument>(`/_elsa/studio/preferences/${dashboardPreferenceNamespace}`, {
