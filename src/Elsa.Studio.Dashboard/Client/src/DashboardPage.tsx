...
export function DashboardPage({ api }: { api: ElsaStudioModuleApi }) {
  ...
  useEffect(() => { let live = true; setDocument(null); void store.load().then(value => { if (live) setDocument({ ...value, value: reconcileDashboardPreferences(value.value, available, pinnedWidgetIds) }); }); return () => { live = false; }; }, [available, pinnedWidgetIds, store]);
  ...
  const update = (mutate: (value: DashboardPreferenceDocument["value"]) => DashboardPreferenceDocument["value"]) => {
    const next = { ...document, value: mutate(document.value) }; setDocument(next); void store.save(next).then(saved => setDocument(current => current === next ? saved : current));
  };
