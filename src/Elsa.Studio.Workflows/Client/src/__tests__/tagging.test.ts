...
import { getWorkflowDefinitionTags, listTagDefinitions, replaceWorkflowDefinitionTags } from "../api/tagging";
...
describe("workflow definition marker tags", () => {
  it("uses advertised tagging and definition-tag links without guessing routes", async () => {
    ...
    } : url === "/tagging/definitions" ? { items: [{ id: "tag-prod", canonicalKey: "environment", displayName: "Environment", status: "Active" }] } : {
    ...
    await expect(listTagDefinitions(value)).resolves.toMatchObject({ items: [{ id: "tag-prod", key: "environment" }] });
  ...
  it("maps canonical catalog fields and sends their canonical spelling on mutations", async () => {
    const postJson = vi.fn().mockResolvedValue({ id: "tag-risk", canonicalKey: "risk", displayName: "Risk", status: "Active" });
    ...
    await expect(createTagDefinition(value, { canonicalKey: "risk", displayName: "Risk" })).resolves.toMatchObject({ key: "risk", deleted: false });
