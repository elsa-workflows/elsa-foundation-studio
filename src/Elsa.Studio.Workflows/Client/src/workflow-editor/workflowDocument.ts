...
export type WorkflowDocumentAction =
  ...
  | { type: "slotEntered"; frame: ScopeFrame }
...
export function workflowDocumentReducer(state: WorkflowDocumentState, action: WorkflowDocumentAction): WorkflowDocumentState {
  switch (action.type) {
    ...
    case "slotEntered":
      return { ...state, frames: [...state.frames, action.frame], selectedNodeId: null };
    case "testRunStarted":
...
export function useWorkflowDocument() {
  ...
  const actions = useMemo(() => ({
    ...
    enterSlot(owner: ActivityNode, slotId: string, label: string) {
      dispatch({ type: "slotEntered", frame: { ownerNodeId: owner.nodeId, slotId, label } });
    },
